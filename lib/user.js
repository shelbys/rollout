...
function User(user, rollout) {
  ...
  if ('object' === typeof this.rollout) {
    ...
    this.rollout.client.sadd(name, user[this.rollout._id], function(err, result) {});
  }
}
...
User.prototype.active = function(feature) {
  ...
  return new Promise(function(resolve, reject) {
    var name = this.rollout.name('rollout:users:' + user[this.rollout._id]);
...
User.prototype.activate = function(feature) {
  ...
  return new Promise(function(resolve, reject) {
    var name = self.rollout.name('rollout:users:' + self.user[self.rollout._id]);
