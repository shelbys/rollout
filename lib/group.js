        ...
        self.rollout.client.smembers(name, function (err, members) {
          ...
          return new Promise(function (resolve, reject) {
            async.detectSeries(members, function (member, eachCallback) {
              ...
              } else if (self.name != member) {
                var name = self.rollout.name('rollout:groups');
                self.rollout.client.sismember(name, member, function(err, result) {
                  if (result === 1) {
                    Group.prototype.active.call({ rollout: self.rollout, name: member }, feature).then(eachCallback);
                  } else {
                    eachCallback(false);
                  }
                });
